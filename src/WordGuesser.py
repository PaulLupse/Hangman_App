...
class WordGuesser:
    ...
    def run_input_file(self):
        ...
        # fisierul de intrare trebuie sa fie de tip csv
        if not input_file.name.endswith(".csv"):
            print("Eroare: Tipul fișierului de intrare este invalid.")
            raise InvalidFileException

        output_file.write("etichetă_joc,total_încercări,cuvânt_găsit,secvență_încercări\n")

        index = 0
        for line in input_file:
            word_pair = line.split(',')

            self.validate_pair(word_pair)

            steps,found_word,tried_letters = self.guess_word(word_pair[1], word_pair[2].strip('\n'))
            total_steps_cnt += steps
            output_file.write(f"{word_pair[0]},{steps},{found_word},{' '.join(tried_letters)}\n")
            index +=1

        output_file.write(f"Total pași efectuați: {total_steps_cnt}\n")
        output_file.write(f"Media pașilor efecțuati: {round(total_steps_cnt/index, 2)}")

    @staticmethod
    def validate_pair(word_pair):

        word_pair[1] = word_pair[1].upper()
        word_pair[2] = word_pair[2].upper()

        if word_pair[2][-1] == '\n':
            word_pair[2] = word_pair[2][:-1:]

        if len(word_pair) != 3 or (len(word_pair[1]) != len(word_pair[2])):
            print(f"Eroare: Formatul fișierului de intrare este invalid. Vedeți linia cu eticheta: {word_pair[0]}.")
            raise InvalidFileException

        for char in word_pair[2]:
            if (char not in romanian_letters) and (char not in "*-\n"):
                print(f"Eroare: Caracter '{char}' interzis detectat la linia cu eticheta: {word_pair[0]}.")
                raise ValueError
